...
import { CopyIcon, DownloadIcon, RefreshIcon, FileTextIcon, ChevronDownIcon } from './icons';
    ...
    const doc = new Document({
      sections: [{
        children: report.split('\n').map(line => {
          ...
          return new Paragraph({ children: parts.length > 0 ? parts : [new TextRun(line)] });
        }),
      }],
    });
    ...
    <div className="w-full max-w-4xl bg-gray-800 rounded-xl shadow-2xl flex flex-col overflow-hidden">
      <header className="p-4 border-b border-gray-700 flex justify-between items-center bg-gray-800/50 flex-wrap gap-y-2">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-xl font-bold text-gray-100">AI Generated Report</h2>
        </div>
          ...
          <div className="relative" ref={dropdownRef}>
            ...
            {isDropdownOpen && (
              <div className="absolute right-0 mt-2 w-48 bg-gray-600 rounded-md shadow-lg z-20 border border-gray-500">
                <ul className="py-1">
                  <li><button onClick={handleDownloadTxt} className="w-full text-left flex items-center gap-3 px-4 py-2 text-sm text-gray-200 hover:bg-indigo-500">
                    <FileTextIcon className="w-5 h-5 text-gray-300" /> As Text (.txt)
                  </button></li>
                  <li><button onClick={handleDownloadDocx} className="w-full text-left flex items-center gap-3 px-4 py-2 text-sm text-gray-200 hover:bg-indigo-500">
                    <FileTextIcon className="w-5 h-5 text-blue-400" /> As Word (.docx)
                  </button></li>
                  <li><button onClick={handleDownloadPdf} className="w-full text-left flex items-center gap-3 px-4 py-2 text-sm text-gray-200 hover:bg-indigo-500">
                    <FileTextIcon className="w-5 h-5 text-red-400" /> As PDF (.pdf)
                  </button></li>
                </ul>
              </div>
            )}
